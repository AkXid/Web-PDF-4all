...
description: 'Regeln für React/Next.js TSX-Dateien im PDF/UA Barrierefreiheit Landingpage Projekt'
applyTo: '**/*.tsx'
---
...
- **Einzelunternehmen**: "Ich/mein" statt "Wir/unser" in allen Texten

## Nicht implementieren
- ❌ `EmergencyBanner` – kein Notdienst
- ❌ `TeamSection` – kein Team
- ❌ `GoogleRatingWidget` – nicht gewünscht
- ❌ `GallerySection` – nicht nötig
...
- Nichts hardcoden was unternehmensspezifisch ist

## Bilder
- Immer `next/image` mit `alt` Text
- Hero-Bilder: `priority` Attribut
- `images: { unoptimized: true }` in `next.config.ts` (Pflicht für GitHub Pages)
