# Design-System: PDF/UA Barrierefreiheit – Landingpage

> Dieses Design-System definiert die visuelle Sprache für die Landingpage eines PDF/UA-Barrierefreiheits-Dienstleisters.
> Ziel: Professionell, vertrauenswürdig, seriös – **kein AI-Look**.
...
### Was wir sind:
- **Expertise, nicht Startup.** Die Seite soll aussehen wie von einer seriösen Agentur gebaut – nicht wie ein SaaS-Dashboard.
- **Vertrauen statt Wow.** Ein Unternehmen mit gesetzlichem Barrierefreiheitsbedarf braucht Klarheit und Kompetenz, keine Wow-Effekte.
- **Deutsch, nicht Silicon Valley.** Angemessen, sachlich, professionell – keine übertriebene Lockerheit.
...
| Lila/Violet als Akzent | Dunkelblau + Goldgelb (Seriosität + Qualität) |
...
### Standard-Palette (PDF/UA Barrierefreiheit)

```typescript
const colors = {
  // Primär – Tiefes Blau (Seriosität, Vertrauen, Kompetenz)
  primary: {
  ...
  // Akzent – Warmes Goldgelb (Qualität, Wertigkeit)
  accent: {
    DEFAULT: "#b45309",     // Amber-700 – CTA, Highlights (etwas dunkler als Handwerk)
    light: "#d97706",       // Hover
  ...
  // Status
  success: "#16a34a",       // Grün – Erfolg/Bestätigung, Normkonformität
  info: "#0369a1",          // Blau – Hinweise, Info-Badges
};
```

### Farbpalette für dieses Projekt

| Einsatz | Farbe | Hex | Begründung |
|---|---|---|---|
| **Primary** | Dunkelblau | `#1a365d` | Vertrauen, Seriosität, Kompetenz |
| **Accent / CTA** | Goldgelb | `#b45309` | Qualität, Wertigkeit, Kontrast |
| **Hover Accent** | Helles Amber | `#d97706` | Hover-State für CTA |
| **Hintergrund** | Slate-50 | `#f8fafc` | Alternierende Sektionen |
| **Text** | Slate-900 | `#0f172a` | Headlines |
| **Fließtext** | Slate-700 | `#334155` | Body-Text |
...
─────────────────────────────────────────────────────
HERO            → Full-Width, dunkles Bild/Farb-Hintergrund, weißer Text
                  Layout: Links-ausgerichtet oder zentriert
─────────────────────────────────────────────────────
TRUST BAR       → Schmaler Streifen, bg-primary, weiße Icons+Zahlen
                  Kein eigener Hintergrund-Wechsel, direkt am Hero dran
─────────────────────────────────────────────────────
LEISTUNGEN      → bg-white, Grid 2x2-Spalten, Cards mit Border
                  Layout: Cards mit Icon oben, NICHT seitlich
─────────────────────────────────────────────────────
ÜBER MICH       → bg-slate-50, Split: Text links, Profilfoto rechts
                  Bild NICHT rounded, sondern kantig
                  "Ich/mein" statt "Wir/unser" – Einzelunternehmen
─────────────────────────────────────────────────────
LEAD MAGNET     → bg-primary (dunkel), weißer Text
                  Layout: Zentriert, Formular darunter
─────────────────────────────────────────────────────
KUNDENSTIMMEN   → bg-white, 3er-Grid oder Slider
                  KEIN Google-Logo – eigene Sterne-Darstellung
─────────────────────────────────────────────────────
ZIELGRUPPEN     → bg-slate-50, Badges als Chips
                  "Für wen ich arbeite" (keine regionale Karte)
─────────────────────────────────────────────────────
FAQ             → bg-white, Akkordeon linksbündig, max-w-3xl
─────────────────────────────────────────────────────
CTA BANNER      → bg-primary, zentriert, 2 Buttons
─────────────────────────────────────────────────────
FOOTER          → bg-slate-900, 3-4-Spalten, helle Schrift
```
...
// Kein Emergency-Banner Pulse (kein Notdienst)
...
// ❌ rotate-Animationen
// ❌ Endlos-Animationen auf sichtbaren Elementen
```
...
/* max-w-7xl (1280px) wirkt bei Dienstleistungsseiten oft zu weit/leer */
```
...
// → Alternativ: ruhiger Farbhintergrund bg-primary ohne Bild
// → Bild-Vorschläge: Schreibtisch mit Dokument, Laptop, ruhige abstrakte Darstellung
// → KEIN generisches Stock-Foto mit lächelnder Person
...
// Über-mich Bild:
// → Platzhalter: Neutraler grauer Kasten mit "Ihr Profilfoto" Text
// → Aspekt: 3:4 (Portrait-Format für Einzelperson)
